 ...
 * Florida Fantasy 5 (FL7) — Results Analysis
 * Joomla 5.1.x | PHP 8.1+ | UTF-8 | ES5 JS
 *
 * GAME CONFIG:
 * - game_id    = FL7  (string key)
 * - main balls = 5    (first, second, third, fourth, fifth)
 * - main range = 01..36
 * - bonus balls = 0   (FL7 has no bonus / special ball)
 *
 * UPSTREAM VARIABLES EXPECTED (set by parent view / module context):
 ...
 * =========================================================================
 * GAME CONFIG
 * All logic is driven from this config block — no pick count is hardcoded
 * outside of it. Swap values here to adapt this file to a different game.
 * =========================================================================
 */
$gameConfig = [
    'gameId'       => 'FL7',
    ...
    'bonusMax'     => 0,
    'bonusLabel'   => '',
    'mainColumns'  => ['first', 'second', 'third', 'fourth', 'fifth'],
    'bonusColumns' => [],
];

$gameId      = $gameConfig['gameId'];
$mainColumns = $gameConfig['mainColumns'];
$hasBonus    = ($gameConfig['bonusBalls'] > 0);

/**
 * =========================================================================
 * CONTEXT VARS — safe fallbacks if not provided upstream
 * =========================================================================
 */
...
if ($dbCol === '' || !preg_match('/^[A-Za-z0-9_#]+$/', $dbCol)) {
    echo '<div style="padding:14px;border:1px solid #f1c40f;background:#fff8e1;border-radius:14px;max-width:980px;margin:14px auto;">'
       . '<strong>Configuration error:</strong> Missing or invalid data source table.'
       . '</div>';
    echo HTMLHelper::_('content.prepare', '{loadposition Pick5Wheels}');
    return;
}
...
if ($stateName !== '' && $gName !== '') {
    $doc->setTitle('Results Analysis — ' . $stateName . ' ' . $gName . ' Number Frequency &amp; Draw History | LottoExpert.net');
    $doc->setMetaData('description', 'Analyze ' . $stateName . ' ' . $gName . ' number frequency, recency, active numbers, quiet numbers, and draw history. Verified results and SKAI-powered insights on LottoExpert.net.');
}
...
/**
 * Game-aware repeated-number detection.
 * Accepts the main-column list so it works for any pick format.
 */
...
/**
 * Generic draw fetch using SELECT * so it works for any game table schema.
 */
...
/**
 * Previous occurrence lookup — accepts a column list so it works for both
 * main-ball pools and bonus-ball pools without hardcoding column names.
 */
 ...
 * =========================================================================
 */
$latestRows = leFetchRecentDraws($db, $dbCol, $gameId, 1);
...
/* Extract main balls dynamically based on game config — no hardcoded pick count */
$latestMainBalls = [];
$mainBallPadded  = [];

foreach ($gameConfig['mainColumns'] as $col) {
    $raw               = $lr ? trim((string) ($lr[$col] ?? '')) : '';
    $latestMainBalls[] = $raw;
    $mainBallPadded[]  = lePad2($raw);
}
...
$logo = ($stateAbrev !== '' && $gName !== '')
    ? leResolveLogo($stateAbrev, $gName)
    : ['exists' => false, 'url' => ''];
 ...
 * =========================================================================
 */
$rowsMain = leFetchRecentDraws($db, $dbCol, $gameId, $nodCurrentMain);

[$mainCounts, $mainLastSeenIndex] = leInitRange($gameConfig['mainMin'], $gameConfig['mainMax']);
...
$mainChartLabels   = leBuildNaturalLabels($gameConfig['mainMin'], $gameConfig['mainMax']);
$mainChartValues   = [];
$mainRecencyValues = [];
...
$repeatedNumbers   = leFindRepeatedFromLatest($latestMainBalls, array_slice($rowsMain, 1, 10), $gameConfig['mainColumns']);
$mostActiveSummary = array_slice($topActiveKeys, 0, 3);
$quietSummary      = array_slice($quietestKeys, 0, 3);

/**
 * Window shift comparison (50 draws vs 300 draws)
 */
$window50  = leFetchRecentDraws($db, $dbCol, $gameId, 50);
$window300 = leFetchRecentDraws($db, $dbCol, $gameId, 300);

[$counts50, ] = leInitRange($gameConfig['mainMin'], $gameConfig['mainMax']);
[$counts300, ] = leInitRange($gameConfig['mainMin'], $gameConfig['mainMax']);
...
/**
 * Draw history rows for the current latest draw — main balls only (FL7 has no bonus)
 */
$drawHistoryRows = [];

if ($drawDate !== '') {
    foreach ($latestMainBalls as $ballVal) {
        $ballPadded = lePad2($ballVal);
        $prevDate   = leGetPreviousOccurrenceDate($db, $dbCol, $gameId, $drawDate, $ballVal, $gameConfig['mainColumns']);
        $drawsAgo   = leGetDrawingsSinceDate($db, $dbCol, $gameId, $prevDate, $drawDate);

        $drawHistoryRows[] = [
            'label'    => $ballPadded !== '' ? $ballPadded : '—',
            'prevDate' => $prevDate,
            'drawsAgo' => $drawsAgo,
            'isBonus'  => false,
        ];
...
$jsonLdWebPage = [
    ...
    '@type'       => 'WebPage',
    'name'        => $stateName . ' ' . $gName . ' Results Analysis',
    'description' => 'Number frequency, recency, active numbers, quiet numbers, and draw history for '
                   . $stateName . ' ' . $gName . '. Verified draw results on LottoExpert.net.',
...
/**
 * URL helpers (HTML-escaped, ready for output)
 */
$gId        = $gameId;
$titleState = htmlspecialchars($stateName, ENT_QUOTES, 'UTF-8');
$titleGame  = htmlspecialchars($gName, ENT_QUOTES, 'UTF-8');
$titleSTn   = htmlspecialchars(strtolower($stateAbrev), ENT_QUOTES, 'UTF-8');
?>
<style>
:root{
  --skai-blue:#1C66FF;
...
<div class="skai-page">
      ...
      <div class="skai-hero-top">
        ...
        <div class="skai-hero-copy">
          ...
          <div class="skai-ball-row" aria-label="Latest drawn numbers">
            <?php foreach ($mainBallPadded as $ball) : ?>
              <span class="skai-ball skai-ball--main"><?php echo htmlspecialchars($ball, ENT_QUOTES, 'UTF-8'); ?></span>
            <?php endforeach; ?>
          </div>

          <div class="skai-hero-actions" aria-label="Primary actions">
            <a class="skai-btn skai-btn--primary" href="/picking-winning-numbers/artificial-intelligence/skai-lottery-prediction?game_id=FL7">
              Open SKAI Analysis
            </a>
            <a class="skai-btn skai-btn--secondary" href="/picking-winning-numbers/artificial-intelligence/ai-powered-predictions?game_id=FL7">
              AI Predictions
            </a>
          ...
          <div class="skai-advanced-links" aria-label="Advanced analysis tools">
            <a class="skai-mini-link" href="/picking-winning-numbers/artificial-intelligence/skip-and-hit-analysis?game_id=FL7">Skip &amp; Hit Analysis</a>
            <a class="skai-mini-link" href="/picking-winning-numbers/artificial-intelligence/markov-chain-monte-carlo-mcmc-analysis?gameId=FL7">MCMC Markov Analysis</a>
            <a class="skai-mini-link" href="/all-lottery-heatmaps?gameId=FL7">Heatmap Analysis</a>
          </div>
        </div>
        ...
        <aside class="skai-result-panel" aria-label="Latest draw details">
          ...
          <div class="skai-meta-stack">
            ...
            <?php if ($nextJackpot !== '' && $nextJackpot !== '0' && $nextJackpot !== 'n/a' && is_numeric($nextJackpot) && (float) $nextJackpot > 0) : ?>
              <div class="skai-meta-box">
  ...
  <section id="frequency-deep-dive" class="skai-section" aria-labelledby="frequency-title">
    <div class="skai-section-head">
      <div>
        ...
        <p class="skai-section-sub">
          Move from summary to full reference. The left panel shows the complete main-number distribution across all values 01&ndash;<?php echo str_pad((string) $gameConfig['mainMax'], 2, '0', STR_PAD_LEFT); ?>. The right panel shows the recency distance for each main number across the selected window.
        </p>
      </div>
    </div>
    ...
    <div class="skai-section-body">
      <div class="skai-two-col">
        <div class="skai-card">
          <div class="skai-card-head skai-card-head--horizon">
            ...
            <span class="skai-card-sub">All values 01&ndash;<?php echo str_pad((string) $gameConfig['mainMax'], 2, '0', STR_PAD_LEFT); ?> across the last <?php echo (int) $nodCurrentMain; ?> drawings</span>
          </div>
          <div class="skai-card-body">
            <div class="skai-chart-shell">
        ...
        <div class="skai-card">
          <div class="skai-card-head skai-card-head--ember">
            ...
            <span class="skai-card-sub">Distance since last appearance for each main number (01&ndash;<?php echo str_pad((string) $gameConfig['mainMax'], 2, '0', STR_PAD_LEFT); ?>)</span>
          </div>
          <div class="skai-card-body">
            <div class="skai-chart-shell">
              <div class="skai-chart-frame skai-chart-frame--tall">
      ...
      <div class="skai-note">
        The complete distribution is best used as a reference layer. The overview modules above are optimized for quick reading; this section is optimized for thorough review. Numbers with higher recency values have been absent for more drawings.
      </div>
    </div>
  </section>
  ...
  <section id="tables" class="skai-section" aria-labelledby="tables-title">
    ...
    <div class="skai-controls">
      <form name="fqsearch" method="post" action="/all-us-lotteries/results-analysis?st=<?php echo htmlspecialchars($stateAbrev, ENT_QUOTES, 'UTF-8'); ?>&amp;stn=<?php echo $titleState; ?>&amp;gm=<?php echo $titleGame; ?>#tables">
    ...
    <div class="skai-section-body">
      <div class="skai-two-col">
        <div class="skai-card">
          <div class="skai-card-head skai-card-head--horizon">
            Main numbers table
            <span class="skai-card-sub">Exact counts and recency for values 01&ndash;<?php echo str_pad((string) $gameConfig['mainMax'], 2, '0', STR_PAD_LEFT); ?></span>
          </div>
          ...
          <div class="skai-table-wrap">
            <table id="skai-main-table" class="skai-table" aria-label="Main numbers frequency table">
              ...
              <tbody>
                <?php for ($i = $gameConfig['mainMin']; $i <= $gameConfig['mainMax']; $i++) : ?>
                  <?php
        ...
        <div class="skai-card">
          <div class="skai-card-head skai-card-head--radiant">
            Window shift detail
            <span class="skai-card-sub">How the top numbers change as the draw window broadens</span>
          </div>
          <div class="skai-card-body">
            <div class="skai-window-shift">
              <div class="skai-shift-panel">
              ...
              <?php if (!empty($windowShiftIn)) : ?>
                <div class="skai-shift-panel">
                  <p class="skai-shift-label">Gains prominence at 300 draws</p>
                  <p class="skai-shift-text"><?php echo htmlspecialchars(leCommaList($windowShiftIn), ENT_QUOTES, 'UTF-8'); ?></p>
                </div>
              <?php endif; ?>

              <?php if (!empty($windowShiftOut)) : ?>
                <div class="skai-shift-panel">
                  <p class="skai-shift-label">More concentrated in 50-draw view</p>
                  <p class="skai-shift-text"><?php echo htmlspecialchars(leCommaList($windowShiftOut), ENT_QUOTES, 'UTF-8'); ?></p>
                </div>
              <?php endif; ?>
            </div>
          </div>

          <div class="skai-note" style="margin:0 16px 16px;">
      ...
      <div class="skai-tool-grid">
        <article class="skai-tool">
          ...
          <div class="skai-tool-body">
            ...
            <a class="skai-tool-cta" href="/picking-winning-numbers/artificial-intelligence/skai-lottery-prediction?game_id=FL7">Open SKAI Analysis</a>
          </div>
        </article>
        ...
        <article class="skai-tool">
          ...
          <div class="skai-tool-body">
            ...
            <a class="skai-tool-cta" href="/picking-winning-numbers/artificial-intelligence/ai-powered-predictions?game_id=FL7">Open AI Predictions</a>
          </div>
        </article>
        ...
        <article class="skai-tool">
          ...
          <div class="skai-tool-body">
            ...
            <a class="skai-tool-cta" href="/picking-winning-numbers/artificial-intelligence/skip-and-hit-analysis?game_id=FL7">Open Skip &amp; Hit</a>
          </div>
        </article>
      </div>

      <div class="skai-utility-grid">
        <a class="skai-utility-link" href="/picking-winning-numbers/artificial-intelligence/markov-chain-monte-carlo-mcmc-analysis?gameId=FL7">MCMC Markov Analysis</a>
        <a class="skai-utility-link" href="/all-lottery-heatmaps?gameId=FL7">Heatmap Analysis</a>
        <a class="skai-utility-link" href="/lottery-archives-pick5?gId=<?php echo rawurlencode($gId); ?>&amp;stateName=<?php echo rawurlencode($stateName); ?>&amp;gName=<?php echo rawurlencode($gName); ?>&amp;sTn=<?php echo rawurlencode(strtolower($stateAbrev)); ?>">Lottery Archives</a>
  ...
  <section class="skai-section" aria-labelledby="method-title">
    ...
    <div class="skai-section-body">
      <div class="skai-method-note">
        <strong>Interpretation guidance:</strong> Frequency, recency, and spacing can provide useful context for reviewing draw history, but they should be treated as descriptive signals rather than guarantees. Lottery draws are independent random events. The purpose of this page is to make the recent behavior of <?php echo $titleGame; ?> easier to understand, compare, and carry into deeper SKAI analysis. <strong>Active</strong> numbers have appeared often within the selected window. <strong>Quiet</strong> numbers have the largest gap since their last appearance. Neither status predicts future outcomes.
      </div>
    </div>
  </section>
...
(function () {
  ...
  /**
   * Chart.js loader — tries CDN with integrity, falls back without integrity,
   * then proceeds whether or not the library loaded.
   */
  ...
  function safeCanvas(id) {
    ...
    if (!el) { return null; }
    /* Guard against near-zero width which produces blank charts */
    var parent = el.parentElement;
    if (parent && parent.offsetWidth < 20) { return null; }
    return el;
  }
  ...
  function bindFilters() {
    ...
    function applyFilter(filter) {
      ...
      for (k = 0; k < buttons.length; k++) {
        ...
        if (buttons[k].getAttribute('data-filter') === filter) {
